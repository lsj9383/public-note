...
* 其它层
    * 网光
## 5.2 IP协议
一个IP包最大为`MTU = 1500字节`, 其中包头最小为20字节。当一个包超过了MTU后需要被拆分为多个包。
* IP公网地址
...
## 网关协议
### 1) 内部网管协议
* RIP，基于距离矢量算法的路由协议。通过达到目的地的最少条数来选取最佳路径，【默认30s】向相邻设置发出整个路由表副本。跳数达到15跳后认为不可达。
    ...
    * 反向毒化
* OSPF，一种链路状态协议，OSPF路由器维护一个本地链路状态表，并随时向其他相邻路由器发送链路状态的更新信息。
